...
function analyzeModifiers(text) {
  ...
  let scale = 1.0,
    negated = false;
  if (
    /(つらくない|疲れてない|大丈夫|問題ない|平気|別につら|そんなに(疲れ|つら)|特に(ない|疲れ))/.test(
      t,
    )
  ) {
    negated = true;
...
function heuristicTextScores(t) {
  ...
  if (/イライラ|落ち[込こ]|不安|ストレス|つらい|気[がも]重|やる気|ざわ/.test(t))
    mental += 20;
...
async function scoreFromText(userText, acoustic) {
  ...
  if (!isEmbedReady || !extractor || !defVectors) {
    ...
    try {
      const out = await extractor(cleaned || "特になし", {
        pooling: "mean",
        normalize: true,
      });
      ...
      const total = Math.max(18, Math.min(95, Math.round(100 - avg * 0.88)));
...
async function scoreFromBackend(blob, text) {
  ...
  if (window.lastEmbedRaw) {
    ...
    if (er.healthy !== undefined)
      form.append("sim_healthy", String(er.healthy));
  }
...
async function scoreWithFallback(userText, acoustic, blob) {
  try {
    if (isEmbedReady && extractor && defVectors) {
      const out = await extractor(userText || "特になし", {
        pooling: "mean",
        normalize: true,
      });
      const v = Array.from(out.data);
...
function renderRecovery(sugs) {
  ...
  DOM.recoveryList.innerHTML = sugs
    .map(
      (s, i) =>
        `<div class="bg-white rounded-2xl p-3 shadow-sm border border-slate-100 flex items-center gap-3 slide-up" style="animation-delay:${0.05 * i}s"><div class="w-12 h-12 rounded-xl ${s.bg} flex items-center justify-center shrink-0"><span class="material-icons-outlined" style="color:${s.color}">${s.icon}</span></div><div class="min-w-0"><p class="text-xs font-bold text-slate-700">${s.title}</p><p class="text-[11px] text-slate-400">${s.desc}</p></div></div>`,
    )
    .join("");
}
...
function applyScoreUI(scores) {
  ...
  const b = document.getElementById("scoreBrainVal"),
    m = document.getElementById("scoreMentalVal"),
    p = document.getElementById("scorePhysicalVal");
  if (b)
    b.innerHTML = `${scores.brain}<span class="text-xs font-normal">%</span>`;
  if (m)
    m.innerHTML = `${scores.mental}<span class="text-xs font-normal">%</span>`;
  if (p)
    p.innerHTML = `${scores.physical}<span class="text-xs font-normal">%</span>`;
  requestAnimationFrame(() => {
    const bb = document.getElementById("barBrain"),
      mb = document.getElementById("barMental"),
      pb = document.getElementById("barPhysical");
...
function setupIOSRecognition() {
  ...
  rec.onresult = (e) => {
    iosLastTranscript = e.results[0][0].transcript;
  };
  rec.onerror = (e) => {
...
async function loadModel() {
  try {
    ...
    if (DOM.aiPromptText)
      DOM.aiPromptText.innerHTML = currentOpening.replace(/。/g, "。<br>");
    ...
    if (DOM.aiPromptText)
      DOM.aiPromptText.innerHTML = "読み込み失敗。タップで再試行してください。";
    if (DOM.micBtn) {
      DOM.micBtn.disabled = false;
      DOM.micBtn.classList.remove("opacity-50");
      DOM.micBtn.onclick = () => {
...
async function transcribe(float32) {
  if (IS_IOS) {
    return iosLastTranscript || "（聞き取れませんでした）";
  }
  ...
  return (
    tokenizer.decode(outputs[0], { skip_special_tokens: true }).trim() ||
    "（聞き取れませんでした）"
  );
}
...
function afterSpeakThenRecord() {
  setTimeout(() => {
    if (
      isModelReady &&
      currentTurn < 2 &&
      DOM.viewResultBtn.classList.contains("hidden")
    ) {
      DOM.statusText.textContent = "タップして話す";
      DOM.micHint.textContent = "マイクを押してください";
      DOM.micBtn.disabled = false;
...
async function processTurn() {
  try {
    ...
    const blob = new Blob(audioChunks, { type: mimeType || `audio/${ext}` });
    lastAudioBlob = blob;
    DOM.statusText.textContent = "文字起こし中...";
    let userText = "";
    if (IS_IOS) {
      await new Promise((r) => setTimeout(r, 300));
      userText = iosLastTranscript || "今日は少し疲れました";
    } else {
    ...
    if (currentTurn === 1) {
      ...
      const messages = [
        {
          role: "system",
          content: buildTurnSystem("followup", currentOpening),
        },
        ...conversationLog,
      ];
      const reply =
        (await callLLM(messages)).trim() ||
      ...
      const summary =
        (await callLLM(closeMessages)).trim() || "話してくれてありがとう。";
      ...
      const scores = await scoreWithFallback(
        joined,
        lastAcoustic,
        lastAudioBlob,
      );
...
async function speakAI(text, onEnd, retries = 1) {
  ...
  function finish() {
    DOM.statusText.textContent = prevStatus;
    isSpeaking = false;
    if (isModelReady && DOM.viewResultBtn.classList.contains("hidden")) {
      DOM.micBtn.disabled = false;
      DOM.micBtn.classList.remove("opacity-50");
    }
    if (onEnd) onEnd();
