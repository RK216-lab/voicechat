          ...
          modelId,
          {
            ...
            progress_callback: (p) => {
              if (p.status === "progress" && p.progress) {
                updateProgress(15 + (p.progress / 100) * 55, `${Math.round(15 + (p.progress / 100) * 55)}%`);
              }
            }
          }
        );
...
async function getAcoustic(blob) {
  try {
    ...
    const r = await fetch(`${BACKEND_URL}/extract-features`, { method: "POST", body: fd });
    if (!r.ok) return {};
    return await r.json();
  } catch (e) {
