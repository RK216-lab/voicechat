/* =========================================================
   Restee 自分スキャン scan.js 完全版 vStable-TTS
   ========================================================= */
...
const ASR_MODEL_CANDIDATES = []; // v4: Moonshine完全無効化 - attention_maskバグ回避、Groqのみ

const EMBED_MODEL = "Xenova/paraphrase-multilingual-MiniLM-L12-v2";
const LOCAL_TTS_MODEL = null;
...
function createPlaybackAudio() {
  ...
  playbackAudio.style.position = "fixed";
  playbackAudio.style.left = "-9999px";
  playbackAudio.style.width = "1px";
  playbackAudio.style.height = "1px";
  playbackAudio.style.opacity = "0.01";
...
const RECOVERY = {
  body: [{ icon: "self_improvement", color: "#ea580c", bg: "bg-orange-100", title: "軽いストレッチ", desc: "座ったまま肩・首をゆっくり回す（3 分）" },{ icon: "hot_tub", color: "#0284c7", bg: "bg-sky-100", title: "温活", desc: "ぬるめのお湯で体の芯を温める" },{ icon: "hotel", color: "#7c3aed", bg: "bg-violet-100", title: "短い仮眠", desc: "15〜20 分の昼寝で体を休める" }],
  brain: [{ icon: "visibility", color: "#0891b2", bg: "bg-cyan-100", title: "画面から離れる", desc: "5〜10 分、遠くを見て目と頭を休める" },{ icon: "directions_walk", color: "#16a34a", bg: "bg-green-100", title: "短い散歩", desc: "外の空気を吸って頭を切り替える" },{ icon: "timer", color: "#64748b", bg: "bg-slate-100", title: "ポモドーロ休息", desc: "25 分集中したら 5 分完全に休む" }],
  mental: [{ icon: "air", color: "#2563eb", bg: "bg-blue-100", title: "深呼吸", desc: "4 秒吸って 6 秒吐く呼吸を 5 回" },{ icon: "edit_note", color: "#db2777", bg: "bg-pink-100", title: "気持ちの書き出し", desc: "今気になっていることをメモに出すだけ" },{ icon: "music_note", color: "#9333ea", bg: "bg-purple-100", title: "好きな休息", desc: "何もしない時間を 10 分つくる" }],
  general: [{ icon: "local_drink", color: "#0d9488", bg: "bg-teal-100", title: "水分補給", desc: "常温の水をゆっくり飲む" },{ icon: "bedtime", color: "#4f46e5", bg: "bg-indigo-100", title: "早めの就寝準備", desc: "いつもより 30 分早く寝る準備を始める" }]
};
...
async function importTransformersRobust() {
  ...
  for (const url of TRANSFORMERS_CANDIDATES) {
    try {
      ...
      if (cand?.pipeline) {
        transformersModule = cand;
        console.log("[Transformers] loaded", url);
        return cand;
      }
    } catch (e) {
      lastErr = e;
      console.warn("[Transformers] failed", url, e?.message || e);
    }
  }
...
async function loadEmbedder(pipelineFn, envObj) {
  try {
    if (!pipelineFn) {
      const mod = await importTransformersRobust();
    ...
    console.warn("[Embedding] failed, heuristic fallback", e);
    isEmbedReady = false;
    throw e;
  }
}

async function loadLocalTTS() { console.log("[Local TTS] disabled"); isLocalTTSReady=false; return null; }
...
function simToCategory(v, p) {
  ...
  return 0.7 * Math.max(...sims) + 0.3 * sims.reduce((a, b) => a + b, 0) / sims.length;
}
...
function acousticFatigueProxy(f) {
  ...
  const get = (...keys) => {
    ...
    for (const k of Object.keys(f)) {
      for (const w of keys) {
        if ((k.endsWith(w) || k.includes(w)) && Number.isFinite(Number(f[k]))) return Number(f[k]);
      }
    }
    return null;
  };
  ...
  let sc = 50;
  if (loud != null) sc += (0.35 - loud) * 60;
  if (jitter != null) sc += (jitter - 0.02) * 400;
  if (hnr != null) sc += (8 - hnr) * 2.5;
  return Math.max(20, Math.min(88, Math.round(sc)));
}

function heuristicTextScores(t) {
  let ph = 42, br = 42, me = 42;
  if (/肩 | 首 | 腰 | だるい|重い|体|眠い|目が疲/.test(t)) ph += 22;
  if (/集中 | ぼんやり|頭|忘れ|ミス|判断|モヤ|整理/.test(t)) br += 22;
  if (/イライラ|不安|ストレス|やる気|落ち込|しんどい|つらい|ざわ/.test(t)) me += 22;
  if (/元気 | 調子いい|大丈夫|問題ない|すっきり/.test(t)) { ph -= 18; br -= 18; me -= 18; }
  const mod = analyzeModifiers(t);
  ph = Math.max(15, Math.min(92, Math.round(ph * mod.scale)));
  br = Math.max(15, Math.min(92, Math.round(br * mod.scale)));
  me = Math.max(15, Math.min(92, Math.round(me * mod.scale)));
  if (mod.negated) { ph = Math.min(ph, 40); br = Math.min(br, 40); me = Math.min(me, 40); }
  return { physical: ph, brain: br, mental: me, total: Math.max(22, Math.min(92, Math.round(100 - ((ph + br + me) / 3) * 0.88))) };
}
...
async function scoreFromText(userText, acoustic) {
  ...
  if (!isEmbedReady || !extractor || !defVectors) {
    es = heuristicTextScores(cleaned);
  } else {
    try {
      ...
      let ph = toPct(raw.body - raw.healthy), br = toPct(raw.brain - raw.healthy), me = toPct(raw.mental - raw.healthy);
      ph = Math.max(12, Math.min(93, Math.round(ph * mod.scale)));
      br = Math.max(12, Math.min(93, Math.round(br * mod.scale)));
      me = Math.max(12, Math.min(93, Math.round(me * mod.scale)));
      if (mod.negated) { ph = Math.min(ph, 38); br = Math.min(br, 38); me = Math.min(me, 38); }
      es = { physical: ph, brain: br, mental: me, total: Math.max(18, Math.min(95, Math.round(100 - ((ph + br + me) / 3) * 0.88))), raw };
    } catch {
      es = heuristicTextScores(cleaned);
    }
  }
  ...
  if (ac != null) {
    const bl = e => Math.round(e * 0.8 + ac * 0.2);
    es.physical = bl(es.physical);
    es.brain = bl(es.brain);
    es.mental = bl(es.mental);
    es.total = Math.max(18, Math.min(95, Math.round(100 - ((es.physical + es.brain + es.mental) / 3) * 0.88)));
  }
  return es;
}
...
// v4: Groq Whisper Turbo + Renderスリープ対応リトライ
async function transcribeWithGroq(blob, retries = 3) {
  if (!blob || !blob.size) return "";
  const ext = getAudioExtension(blob);
  for (let attempt=0; attempt<retries; attempt++) {
    try {
      if (attempt>0) {
        if (DOM.statusText) DOM.statusText.textContent = `サーバー起動中...(${attempt}/${retries-1})`;
        await new Promise(r=>setTimeout(r, 4000));
      }
      const fd = new FormData();
      fd.append("file", blob, `recording.${ext}`);
      console.log(`[Groq STT] attempt ${attempt} ${blob.size} bytes`);
      const res = await fetch(`${BACKEND_URL}/transcribe`, {method:"POST", body:fd, mode:"cors", credentials:"omit", cache:"no-store"});
      console.log(`[Groq STT] status ${res.status}`);
      if (!res.ok) {
        const t = await res.text().catch(()=> "");
        console.warn("[Groq STT] not ok", res.status, t.slice(0,200));
      ...
      console.warn(`[Groq STT] attempt ${attempt} failed`, e.message);
      if (attempt===retries-1) return "";
      if (e.message.includes("Failed to fetch")) continue;
    }
  }
  return "";
}

function getAudioExtension_v4(blob) {
...
async function scoreFromBackend(blob, text) {
  ...
  if (window.lastEmbedRaw) {
    ...
    if (er.body !== undefined) fd.append("sim_body", String(er.body));
    if (er.brain !== undefined) fd.append("sim_brain", String(er.brain));
    if (er.mental !== undefined) fd.append("sim_mental", String(er.mental));
    if (er.healthy !== undefined) fd.append("sim_healthy", String(er.healthy));
  }
  const res = await fetch(`${BACKEND_URL}/predict-fatigue`, { method: "POST", body: fd });
...
async function scoreWithFallback(userText, acoustic, blob) {
  ...
  if (blob) {
    try {
      if (DOM.statusText) DOM.statusText.textContent = "分析中...";
      return await scoreFromBackend(blob, userText);
    } catch (e) {
      console.warn("[backend fallback to local]", e);
    }
  }
...
function pickRecovery(scores) {
  ...
  for (const it of list) {
    if (!seen.has(it.title)) { seen.add(it.title); uniq.push(it); }
  }
...
function titleFromScores(scores) {
  ...
  const [n, v] = e[0];
  if (v >= 72) return `${n}の強い疲れ`;
  if (v >= 55) return `${n}寄りの疲れ`;
...
async function loadModel() {
  try {
    ...
    // v4: ローカルASRは読み込まない - Groq Whisper Turboのみ使用
    console.log("[ASR] skip Moonshine (buggy), using Groq");
...
async function getAcoustic(blob) {
  try {
    ...
    const urls = [`${BACKEND_URL}/extract-features`, `${BACKEND_URL}/api/extract-features`];
    for (const url of urls) {
      try {
        const r = await fetch(url, { method: "POST", body: fd, mode:"cors", credentials:"omit" });
        if (!r.ok) { console.warn("[OpenSMILE] not ok", r.status); continue; }
        return await r.json();
      } catch(e){ console.warn("[OpenSMILE] fetch failed", e.message); }
    }
    return {};
  } catch (e) {
...
function updateWaveFromRms(rms) {
  ...
  DOM.waveDots.forEach((dot, i) => {
    ...
    dot.style.opacity = String(0.35 + level * 0.65);
  });
}
...
async function startRecording() {
  ...
  if (!isMediaRecorderSupported() && !recognition) {
    if (DOM.statusText) DOM.statusText.textContent = "この端末は録音に未対応です";
    return;
  }
  await unlockAudioContext();
  ...
  if (!isMediaRecorderSupported() && recognition) {
    ...
    if (DOM.statusText) DOM.statusText.textContent = "あなたのお話を聞いています...";
    setMicUI(true);
  ...
  try {
    ...
    mediaRecorder.onstop = async () => {
      ...
      if (analyserContext) {
        try { if (analyserContext.state !== "closed") await analyserContext.close(); } catch {}
    ...
    const AC = window.AudioContext || window.webkitAudioContext;
    if (AC) {
      analyserContext = new AC();
      if (analyserContext.state === "suspended") await analyserContext.resume();
      const src = analyserContext.createMediaStreamSource(stream);
    ...
    console.error(e);
    isRecording = false;
    if (DOM.statusText) DOM.statusText.textContent = "マイクの許可が必要です。設定を確認してください";
    setMicUI(false);
  }
}
...
function stopRecording() {
  ...
  if (!isMediaRecorderSupported()) {
    ...
    if (DOM.statusText) DOM.statusText.textContent = "認識中...";
    setMicUI(false);
    if (recordingTimeoutId) { clearTimeout(recordingTimeoutId); recordingTimeoutId = null; }
    (async () => {
      await stopIOSRecognitionAsync();
...
function loopMonitor() {
  ...
  const th = AVOID_CONCURRENCY ? SILENCE_THRESHOLD * 1.5 : SILENCE_THRESHOLD;
  const ms = AVOID_CONCURRENCY ? SILENCE_MS + 1000 : SILENCE_MS;
  if (rms < th) {
    if (!silenceStart) silenceStart = Date.now();
    else if (Date.now() - silenceStart > ms) { stopRecording(); return; }
...
")}` }
      ...
      const te = document.getElementById("fatigueTitle"), de = document.getElementById("fatigueDetailText");
      ...
      if (window.ResteeApp?.saveScanResult) {
        ...
        localStorage.setItem("restee_last_scan", JSON.stringify({ ...saveData, timestamp: new Date().toISOString(), dateStr: new Date().toLocaleString("ja-JP") }));
      }
      updateProgress(100, "100%");
...
async function speakWithBackend(text) {
  ...
  const urls = [
    ...
    `${BACKEND_URL}/tts?text=${encodeURIComponent(text)}`,
  ];
...
async function speakAI(text, onEnd) {
  ...
  isSpeaking = true;
  const safeEnd = ()=>{ isSpeaking=false; try{ onEnd?.(); }catch{} };
