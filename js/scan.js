...
/*
 * iOSではAudioContextだけunlockしても
 * HTMLAudioElementの再生が許可されないケースがある。
 *
 * そこで
 * 1. AudioContext
 * 2. HTMLAudioElement
  ...
  try {
    ...
    if (audio && audio.paused) {
      ...
      const silentWav =
        ...
        "UklGRiQAAABXQVZFZm10IBAAAAABAAEAQB8AAIAfAAABAAgAZGF0YQAAAAA=";
...
function preprocessText(raw) {
  ...
  t = t.replace(
    /^(えっと|えー|あの|その|まあ|なんか|こう|はい|うん|うーん)+/g,
    ""
  );
...
function analyzeModifiers(text) {
  ...
  if (
    /(つらくない|疲れてない|疲れていない|大丈夫|問題ない|平気|別につら|そんなに(疲れ|つら)|特に(ない|疲れ))/.test(t)
  ) {
    negated = true;
    scale *= 0.45;
  }
  ...
  if (
    /(かなり|とても|すごく|めっちゃ|本当に|ひどく|限界|もう無理)/.test(t)
  ) {
    scale *= 1.28;
  }
...
async function loadLocalTTS() {
  ...
  try {
    ...
    localTTSPipeline = await pipeline(
      "text-to-speech",
      LOCAL_TTS_MODEL,
      {
        dtype: "q8",
...
function heuristicTextScores(t) {
  ...
  if (
    /だる|重い|肩こり|腰痛|眠い|体力|こわば|目が疲/.test(t)
  ) {
    physical += 22;
  }

  if (
    /集中|ぼんやり|ミス|頭|忘れ|整理|判断|モヤ/.test(t)
  ) {
    brain += 22;
  }

  if (
    /イライラ|落ち[込こ]|不安|ストレス|つらい|気[がも]重|やる気|ざわ/.test(t)
  ) {
    mental += 20;
  }

  if (
    /元気|調子いい|大丈夫|問題ない|すっきり/.test(t)
  ) {
    physical -= 18;
    brain -= 18;
    mental -= 18;
...
async function scoreFromText(
  userText,
  acoustic
) {
  const cleaned =
    preprocessText(userText);
  ...
  if (
    ...
    try {
      ...
      const raw = {
        ...
        mental: simToCategory(
          v,
          defVectors.mental
        ),

        healthy: simToCategory(
          v,
          defVectors.healthy
        )
      };
  ...
  if (
    ...
    embedScores.physical =
      Math.min(
        93,
        embedScores.physical + 18
      );

    embedScores.brain =
      Math.min(
        93,
        embedScores.brain + 14
      );

    embedScores.mental =
      Math.min(
        93,
        embedScores.mental + 16
      );
  ...
  const ac =
    acousticFatigueProxy(
      acoustic || lastAcoustic
    );
...
async function scoreFromBackend(
  blob,
  text
) {
  if (!blob) {
  ...
  if (window.lastEmbedRaw) {
    ...
    if (er.brain !== undefined)
      form.append(
        "sim_brain",
        String(er.brain)
      );

    if (er.mental !== undefined)
      form.append(
        "sim_mental",
        String(er.mental)
      );

    if (er.healthy !== undefined)
      form.append(
        "sim_healthy",
        String(er.healthy)
      );
...
function renderRecovery(sugs) {
  ...
  DOM.recoveryList.innerHTML =
    sugs
      .map(
        (s, i) => `
<div class="bg-white rounded-2xl p-3 shadow-sm border border-slate-100 flex items-center gap-3 slide-up"
...
function applyScoreUI(scores) {
  ...
  if (b) {
    b.innerHTML =
      `${scores.brain}<span class="text-xs font-normal">%</span>`;
  }

  if (m) {
    m.innerHTML =
      `${scores.mental}<span class="text-xs font-normal">%</span>`;
  }

  if (p) {
    p.innerHTML =
      `${scores.physical}<span class="text-xs font-normal">%</span>`;
  }
...
async function loadModel() {
  try {
     ...
     * iOS
     *
     * iOSではMoonshine WASMを無理にロードせず、
     * Safari SpeechRecognitionを使用。
     */
    if (IS_IOS) {
      recognition =
        setupIOSRecognition();
    ...
    /*
     * Moonshineが失敗しても、
     * iOS SpeechRecognition / heuristicで動作させる
     */
    isModelReady = true;
    ...
    if (DOM.aiPromptText) {
      DOM.aiPromptText.innerHTML =
        currentOpening.replace(
          /。/g,
          "。<br>"
        );
    }
...
function setMicUI(rec) {
  ...
  if (rec) {
    DOM.micInnerIcon.textContent =
      "stop";

    DOM.micInnerIcon.className =
      "material-icons stop-icon";
...
function updateWaveFromRms(
  ...
  DOM.waveDots.forEach(
    (dot, i) => {
      const center =
        (DOM.waveDots.length - 1) /
        2;
  ...
  try {
    ...
    mediaRecorder.onstop =
      async () => {
        ...
        if (
          audioContext &&
          audioContext !== playbackAudio
        ) {
          /*
           * 録音用contextを閉じる。
           * playbackAudioはHTMLAudioなので別。
           */
          try {
            if (
              audioContext.state !==
    ...
    DOM.waveContainer.classList.remove(
      "hidden"
    );
...
function stopRecording() {
  ...
  DOM.waveContainer.classList.add(
    "hidden"
  );
...
function afterSpeakThenRecord() {
  setTimeout(() => {
    if (
      isModelReady &&
      currentTurn < 2 &&
      DOM.viewResultBtn.classList.contains(
        "hidden"
      )
    ) {
      DOM.statusText.textContent =
...
${conversationLog
  ...
  .join("\n")}`
      ...
      if (
        ...
        window.ResteeApp.saveScanResult(
          saveData
        );
      } else {
      ...
      DOM.viewResultBtn.classList.remove(
        "hidden"
      );
 ...
 * Render:
 *
 * GET /tts?text=...&voice=ja-JP-NanamiNeural
 *
 * を使用。
 *
 * Edge TTSを最優先。
...
async function speakWithLocalMMS(
  text
) {
  if (
    !isLocalTTSReady
...
function initScan() {
  ...
  if (DOM.viewResultBtn) {
    DOM.viewResultBtn.addEventListener(
      "click",
      () => {
        DOM.scanScreen.classList.add(
          "hidden"
        );

        DOM.resultScreen.classList.remove(
          "hidden"
        );

        DOM.resultScreen.classList.add(
          "entering"
        );
      }
    );
